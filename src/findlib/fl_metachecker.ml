  ...
  if has_bad_lean_decl then
    ...
    if is_declared_lean then (
      ...
      let archive_byte_plugin =
        try Some(lookup "archive" ["byte"; "plugin"] meta.pkg_defs)
        with Not_found -> None in
      ...
      let archive_native_plugin =
        try Some(lookup "archive" ["native"; "plugin"] meta.pkg_defs)
        with Not_found -> None in
      ...
      if reasons_1 = [] then (
        ...
        [ token_if
          ...
          token_if
            ...
            (Incompat_bad_archive("byte,plugin","missing file lib.cma"));
          token_if
            (archive_byte_plugin = None && bare.bare_byte_archive <> None)
            (Incompat_bad_archive("byte,plugin","missing entry: lib.cma"));
          token_if
          ...
          token_if
            ...
            (Incompat_bad_archive("native,plugin","missing file lib.cmxs"));
          token_if
            (archive_native_plugin = None && bare.bare_shared_archive <> None)
            (Incompat_bad_archive("native,plugin","missing entry: lib.cmxs"));
          token_if
...
let token_to_text token =
  match token with
    ...
    | Incompat_bad_archive(which,what) ->
        sprintf " - bad variable 'archive(%s)': %s\n" which what
    | Incompat_bad_requires file ->
        sprintf " - bad setting for the 'requires' variable which differs from what the file %s specifies.\n" file
    | Incompat_inline_subpackage ->
